...
def main():
  ...
  y_train = dftrain.pop('survived') # we are dropping the survived column from dftrain
  y_eval = dfeval.pop('survived')
  
  print(dftrain['age']) # show the values for the age column
  print(dftrain.loc[0]) # the row values for the first column
  print(dftrain.describe()) # gives an overall information on the data (mean, std, min etc.)
